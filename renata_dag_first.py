...
def dag_renata():
    ...
    def extract1():
        query = """select 
                ...
                from simulator_20230120.feed_actions
                group by user_id"""
        df_cube1 = ch_get_df(query=query)
        return df_cube1
    ...
    def extract2():
        query = """select event_date, user_id, os, gender, age, t1.messages_received, t1.messages_sent, t2.users_received, t2.users_sent from
            ...
            format TSVWithNames"""
        df_cube2 = ch_get_df(query=query)
        return df_cube2

    @task()
    def transform_join_table(df_cube1, df_cube2):
        df_cubes = df_cube1.merge(df_cube2, on='user_id')
    return df_cubes
    
    @task()
    def transform_os(df_cubes):
        df_cubes_os = df_cubes.groupby(['os']).sum().reset_index()
    ...
    def transform_age(df_cubes):
        df_cubes_age = df_cubes.groupby(['age']).sum().reset_index()
    ...
    def transform_gender(df_cubes):
        df_cubes_gender = df_cubes.groupby(['gender']).sum().reset_index()
    ...
    def total_table(df_cubes_os, df_cubes_age, df_cubes_gender):
        ...
        print(df_total.to_csv(index=False, sep='\t')
    ...
    def load (total_table):
        ...
        n = pandahouse.to_clickhouse(df_total, table_name, index=False, connection=connection_write)
        print(f'count of rows: {n}')

    df_cube1 = extract1()
    df_cube2 = extract2()
    df_cubes = transfrom_join(df_cube1, df_cube2)
    df_cubes_os = transfrom_os(df_cubes)
    df_cubes_gender = transform_gender(df_cubes)
    df_cubes_age = transfrom_age(df_cubes)
